...
task :deploy do
	sh "ssh -p #{MASTER_PORT} #{USER}@#{HOST} 'rm -rf /etc/puppet/modules/puppet-logstash'"
	sh "#{SCP} -P #{MASTER_PORT} -i ~/.ssh/id_puppettest ~/projects/puppet-logstash #{USER}@#{HOST}:/etc/puppet/modules/puppet-logstash"
